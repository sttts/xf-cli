...
type ListThreadsIParticipatedCmd struct {
...
func (cmd *ListThreadsIParticipatedCmd) Run(app *App) error {
	client, session, err := app.login()
	if err != nil {
		return err
	}

	result, err := scraper.ListThreadsIParticipated(client, session, cmd.Page, cmd.Limit)
	if err != nil {
		return err
	}
